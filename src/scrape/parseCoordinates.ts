...
const ddRegex = new RegExp(
  /^([+-]?\d+\.\d+) *[d°]? *([NnSs]?)[, /|]*([+-]?\d+\.\d+) *[d°]? *([EwWw]?)$/,
);
// eslint-disable-next-line max-len
const dmsRegex = new RegExp(
  /^([NnSs]?)([+-]?\d{0,3}) *[d:°ºdD]? *(\d{0,3}\.?\d{0,6}) *[:'′mM]? *(\d{0,3}\.?\d{0,6})? *[:"″sS]? *([NnSs]?)[, /|]*([EeWw]?)([+-]?\d{0,3}\.?\d{0,6}) *[d:°ºdD]? *(\d{0,3}\.?\d{0,6}) *[:'’′mM]? *(\d{0,3}\.?\d{0,6})? *[:"”″sS]? *([EwWw]?)$/,
);
