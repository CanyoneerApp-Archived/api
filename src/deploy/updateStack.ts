...
export async function updateStack(options: UpdateStackOptions) {
  ...
  try {
    ...
    if (!stack || stack.StackStatus === 'DELETE_COMPLETE') {
      ...
      const stopLoggingStackEvents = logStackEvents()
  ...
  async function getStackOutputs() {
    ...
    return Object.fromEntries(
      response.Stacks?.[0].Outputs?.map(o => [o.OutputKey, o.OutputValue]) ?? []
    );
  }

  function logStackEvents() {
    ...
    const interval = setInterval(async () => {
      ...
      events.StackEvents?.forEach(event => {
        ...
        console.log(
          chalk.dim(
            `  ${event.ResourceType} ${event.LogicalResourceId} ${event.ResourceStatus} ${event.ResourceStatusReason ?? ''
            }`,
          ),
        );
      });
